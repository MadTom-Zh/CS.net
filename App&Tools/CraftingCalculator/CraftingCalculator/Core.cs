...
namespace MadTomDev.App
{
    public class Core
    {
        ...
        internal Recipe? FindRecipe(Guid recipeId)
        {
            switch (WorkMode)
            {
                case WorkModes.None:
        ...
        internal List<Recipe> FindRecipe_byProcessor(Guid processorId)
        {
            List<Recipes.Recipe> result = new List<Recipe>();
            switch (WorkMode)
            {
                case WorkModes.None:
                    return null;
        ...
        internal List<Recipe> FindRecipes_byInputs(Guid[] inputIds)
        {
            List<Recipes.Recipe> result = new List<Recipe>();
                ...
                case WorkModes.Calculation:
                    {
                        if (recipesFinal != null)
                        {
                            foreach (Recipes.Recipe r in recipesFinal.list)
                            {
                                if (CheckRecipe(r, inputIds, true))
                                {
                                    result.Add(r);
                                }
                            }
            ...
            bool CheckRecipe(Recipes.Recipe r, Guid[] idList, bool inputsOrOutputs)
            {
                foreach (Guid id in idList)
                {
                    if (inputsOrOutputs)
                    {
                        if (r.inputs.Find(a => a.thing?.id == id) == null)
                        {
                            return false;
                        }
                    }
                    ...
                    {
                        if (r.outputs.Find(a => a.thing?.id == id) == null)
                        {
                            return false;
                        }
                    }
        ...
        internal List<Recipe> FindRecips_byOutputs(Guid[] outputIds)
        {
            List<Recipes.Recipe> result = new List<Recipe>();
                ...
                case WorkModes.Calculation:
                    {
                        if (recipesFinal != null)
                        {
                            foreach (Recipes.Recipe r in recipesFinal.list)
                            {
                                if (CheckRecipe(r, outputIds, false))
                                {
                                    result.Add(r);
                                }
                            }
        ...
        public void ReInitCalculation(TreeViewNodeModelScene scene)
        {
            ...
            HashSet<Guid> excludedThingIdList = new HashSet<Guid>();
            if (thingsFinal != null)
            {
                // record excluded things id
                foreach (Things.Thing t in thingsFinal.list)
                {
                    if (t.isExcluded == true)
                    {
                        excludedThingIdList.Add(t.id);
                    }
                }
            ...
            // 2024 0801
            // 如果配方中出现禁用物品，则禁用配方，配方输出也禁用，以此类推；
            if (recipesFinal != null)
            {
                while (RemoveRecipeWithExcludedThing(ref recipesFinal.list, ref excludedThingIdList))
                { }
            }
            ...
            // 因为去除了部分物品，所以之后再初始化物品；
            calculationThings.Clear();
            if (thingsFinal != null)
            {
                for (int i = thingsFinal.list.Count - 1; i >= 0; --i)
                {
                    if (excludedThingIdList.Contains(thingsFinal.list[i].id))
                    {
                        thingsFinal.list.RemoveAt(i);
                    }
                }
                foreach (Things.Thing t in thingsFinal.list)
                {
            ...
            bool RemoveRecipeWithExcludedThing(ref List<Recipe> rList, ref HashSet<Guid> tIdList)
            {
                Recipe r;
                bool toRemove, isRemoved = false;
                List<Guid?> outputTId;
                for (int i = rList.Count - 1; i >= 0; --i)
                {
                    ...
                    if (r.id.ToString().EndsWith("a4ad"))
                    {
                        ;
                    }
                    ...
                    if (r.processor == null
                        || tIdList.Contains((Guid)r.processor))
                    {
                        toRemove = true;
                    }
                    if (!toRemove)
                    {
                        foreach (Recipe.PIOItem aItem in r.accessories)
                        {
                            if (aItem.isExcluded || aItem.thing == null)
                            {
                                toRemove = true;
                                break;
                            }
                            if (aItem.thing != null
                                && tIdList.Contains(aItem.thing.id))
                            {
                                toRemove = true;
                                break;
                            }
                    ...
                    if (!toRemove)
                    {
                        foreach (Recipe.PIOItem aItem in r.inputs)
                        {
                            if (aItem.isExcluded || aItem.thing == null)
                            {
                                toRemove = true;
                    ...
                    if (!toRemove)
                    {
                        foreach (Recipe.PIOItem aItem in r.outputs)
                        {
                            if (aItem.isExcluded || aItem.thing == null)
                            {
                                toRemove = true;
                    ...
                    if (toRemove)
                    {
                        outputTId = r.outputs.Select(a => a.thing?.id).ToList();
                        rList.RemoveAt(i);
                        CheckAddExcludedTId(ref tIdList, ref rList, outputTId);
                        isRemoved = true;
                    }
                }
                return isRemoved;
            }
            void CheckAddExcludedTId(ref HashSet<Guid> excludedTIdList, ref List<Recipe> recipeList, List<Guid?> testOutputTIdList)
            {
                for (int i = testOutputTIdList.Count - 1; i >= 0; --i)
                {
                    if (testOutputTIdList[i] == null)
                    {
                        testOutputTIdList.RemoveAt(i);
                    }
                }
                if (testOutputTIdList.Count == 0)
                {
                    return;
                }
                foreach (Recipe r in recipeList)
                {
                    foreach (Guid? g in r.outputs.Select(a => a.thing?.id).ToList())
                    {
                        if (g != null && testOutputTIdList.Contains(g))
                        {
                            testOutputTIdList.Remove(g);
                            if (testOutputTIdList.Count <= 0)
                            {
                                break;
                            }
                ...
                if (testOutputTIdList.Count > 0)
                {
                    foreach (Guid? g in testOutputTIdList)
                    {
                        excludedTIdList.Add((Guid)g);
                    }
                }
            }
        ...
        public Calculator NewCalculate(Guid productId, List<Guid> filterRecipeIDList,
            List<Guid> filterAccessoryIDList, List<Guid> filterChannelIDList,
            List<Guid> filterInputIDList, List<Guid> filterOutputIDList)
        {
            Calculator cal = new Calculator(productId, filterRecipeIDList,
        ...
        public static Color ColorProcedure = Colors.Gold;
        public static SolidColorBrush ColorBrushProcedure = Brushes.Gold;
        public static Color ColorAccessories = Colors.Magenta;
        public static SolidColorBrush ColorBrushAccessories = Brushes.Magenta;
        public static Color ColorInput = Colors.Orange;
        public static SolidColorBrush ColorBrushInput = Brushes.Orange;
        public static Color ColorOutput = Colors.Aqua;
        public static SolidColorBrush ColorBrushOutput = Brushes.Aqua;
        public static Color ColorChannel = Colors.Lime;
        public static SolidColorBrush ColorBrushChannel = Brushes.Lime;
        #endregion
        ...
        private SettingsTxt _setting;

        public enum DefaultSpeedUnits
        {
            UnitsPerSec, UnitsPerMin,UnitsPerHour,
        }
        public DefaultSpeedUnits? DefaultSpeedUnit_pre { private set; get; }
        private DefaultSpeedUnits? _DefaultSpeedUnit = null;
        public DefaultSpeedUnits DefaultSpeedUnit
        {
            get
            {
                if (_DefaultSpeedUnit is not null)
                {
                    return _DefaultSpeedUnit.Value;
                }
                else
                {
                    string key = nameof(DefaultSpeedUnit);
                    if (Enum.TryParse(
                        typeof(DefaultSpeedUnits),
                        _setting[key],
                        out object? v))
                    {
                        _DefaultSpeedUnit = (DefaultSpeedUnits)v;
                    }
                    else
                    {
                        _DefaultSpeedUnit = DefaultSpeedUnits.UnitsPerSec;
                        DefaultSpeedUnit = DefaultSpeedUnits.UnitsPerSec;
                    }
                }
                return _DefaultSpeedUnit.Value;
            }
            set
            {
                DefaultSpeedUnit_pre = _DefaultSpeedUnit;
                _DefaultSpeedUnit = value;
                string key = nameof(DefaultSpeedUnit);
                _setting[key] = value.ToString();
                _setting.Save();
            }
        }
        internal decimal GetChangedUnitMultiple()
        {
            switch (DefaultSpeedUnit_pre, DefaultSpeedUnit)
            {
                case (DefaultSpeedUnits.UnitsPerSec, DefaultSpeedUnits.UnitsPerMin):
                    return 60;
                case (DefaultSpeedUnits.UnitsPerSec, DefaultSpeedUnits.UnitsPerHour):
                    return 3600;
                case (DefaultSpeedUnits.UnitsPerMin, DefaultSpeedUnits.UnitsPerSec):
                    return 1 / 60;
                case (DefaultSpeedUnits.UnitsPerMin, DefaultSpeedUnits.UnitsPerHour):
                    return 60;
                case (DefaultSpeedUnits.UnitsPerHour, DefaultSpeedUnits.UnitsPerSec):
                    return 1 / 3600;
                case (DefaultSpeedUnits.UnitsPerHour, DefaultSpeedUnits.UnitsPerMin):
                    return 1 / 60;
                default:
                    return 1;
            }
        ...
        public Task SetCursorWait(Window window, int waitTime = 10)
        {
            window.Cursor = Cursors.AppStarting;
            return Task.Delay(waitTime);
        }
        public Task SetCursorArrow(Window window, int waitTime = 10)
        {
            window.Cursor = Cursors.Arrow;
            return Task.Delay(waitTime);
        }

        public SubWindows.WindowSelectThings? ShowSelectThingsWin(Window owner, bool isMultiSelect = false)
        {
            SubWindows.WindowSelectThings selectWin = new SubWindows.WindowSelectThings()
            {
                Owner = owner,
                DataGridSource = WorkMode == WorkModes.Maintaince ? maintainThings : calculationThings,
        ...
        public Data.SettingsLanguage settingLanguage = Data.SettingsLanguage.GetInstance();
        public string SelectedLanguage { private set; get; } = "";
        public string[] GetLanguageList()
        {
            return settingLanguage.ListStoredLanguages();
        }
        public void TrySetLanguage(Window win, string? language = null)
        {
            if (language == null)
            {
            ...
            settingLanguage.TrySetLang(Application.Current.Resources, language, false);
        ...
        {
            if (!File.Exists(SceneMgr.FILENAME_SELECTEDLANGUAGE))
            {
                return;
            }
            SelectedLanguage = File.ReadAllText(SceneMgr.FILENAME_SELECTEDLANGUAGE);
        }
        public void SaveSelectedLanguage()
        {
            File.WriteAllText(SceneMgr.FILENAME_SELECTEDLANGUAGE, SelectedLanguage);
        }
