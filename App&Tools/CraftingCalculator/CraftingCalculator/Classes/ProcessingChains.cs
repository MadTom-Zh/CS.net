...
namespace MadTomDev.App.Classes
{
    public class ProcessingChains
    {
        ...
        public List<Guid> filterRecipeIDList;
        public List<Guid> filterAccessoryIDList;
        public List<Guid> filterChannelIDList;
        public List<Guid> filterInputIDList;
        public List<Guid> filterOutputIDList;
            ...
            List<Guid> filterInputIDList, List<Guid> filterOutputIDList)
        {
            ...
            this.filterRecipeIDList = filterRecipeIDList;
            this.filterAccessoryIDList = filterAccessoryIDList;
            this.filterChannelIDList = filterChannelIDList;
            this.filterInputIDList = filterInputIDList;
            this.filterOutputIDList = filterOutputIDList;


            List<SearchHelper> shList = new List<SearchHelper>();
            SearchHelper curSH;
            bool foundFilterAccessory;
            foreach (Recipes.Recipe r in GetRecipes(targetProduct))
            {
                if (CheckHasFilterProcessorOrAccessory(r))
                {
                    continue;
                }
            ...
            {
                if (CheckHasFilterInputOrOutput(shList[i]))
                {
                    shList.RemoveAt(i);
                }
            }
            ...
            foreach (SearchHelper sh in shList)
            {
                ...
                if (sh.IsLinksPossible)
                {
                    foreach (Guid cId in filterChannelIDList)
                    {
                        foundChannel = core.FindChannel(cId);
                        if (foundChannel != null)
                        {
                            sh.AddFilterOutChannel(foundChannel);
                        }
                    }
                }
            }
            ...
            {
                //allProcesses
                for ( i = 0, iv = sh.allProcesses.Count - 1,jv = iv+1; i < iv; ++i)
                {
                    pNode1 = sh.allProcesses[i];
                    for (j = i+1;j<jv;++j)
                    {
                        pNode2 = sh.allProcesses[j];
                        if (pNode1.recipe.processor == pNode2.recipe.processor)
                        {
                            ...
                            sh.allProcesses.Remove(pNode2);
                            sh.allProcesses.Insert(i+1, pNode2);
                            break;
                        }
                    }
                }
        ...
        private bool CheckHasFilterProcessorOrAccessory(Recipes.Recipe r)
        {
            if (r.processor == null)
            {
                throw SearchHelper.Error_Recipe_Processer_isNull(r.name);
            }
            if (filterRecipeIDList.Contains((Guid)r.id))
            {
                return true;
            }
            foreach (Guid rId in r.accessories.Select(a => a.thing.id))
            {
                if (filterAccessoryIDList.Contains(rId))
                {
                    return true;
                }
            }
        ...
        private bool CheckHasFilterInputOrOutput(SearchHelper sh)
        {
            foreach (Guid sId in sh.allSources.Select(a => a.DataId))
            {
                if (filterInputIDList.Contains(sId))
                {
                    return true;
                }
            }
            foreach (Guid pId in sh.allFinalProducts.Select(a => a.DataId))
            {
                if (filterOutputIDList.Contains(pId))
                {
                    return true;
                }
            }
                ...
                for (int i = 0, iv = pNode.recipe.inputs.Count; i < iv; ++i)
                {
                    ...
                    foreach (Recipes.Recipe r in GetRecipes(input.thing))
                    {
                        ...
                        if (existPNode != null)
                        {
                            ...
                            if ((availableOutRate - inRateNeeded) < aboutZero)
                            {
                                // equal
        ...
        public class ProcessingNode : ProcessingNodeBase
        {
            ...
            public void CalNSetBaseMultiple(int outputIndex, decimal outputRate)
            {
                ...
                // 按产品输出速度，计算处理速度
                decimal newMultiple = outputRate / SearchHelper.CalBaseSpeed(pioItem.quantity.ValueCurrentInGeneral, recipe.period, 1, recipe.name);
                if (newMultiple < aboutZero)
                {
                    ;
                }
                baseMultiple = newMultiple;
            }
            ...
            public List<Things.Thing> AllAccessories_noDuplicates
            {
                get
                {
                    if (_AllAccessories_noDuplicates.Count == 0)
                    {
                        ...
                        SearchHelper.EliminateDuplication<Things.Thing>(ref _AllAccessories_noDuplicates);
                    }
        ...
        public class SearchHelper : SearchResult
        {
            ...
            internal bool LinkProductToInput(ProcessingNode rightPNode, Things.Thing product)
            {
            ...
            public bool TrySimplify01()
            {
                ...
                for (int i = 0, iv = allSources.Count; i < iv; ++i)
                {
                    ...
                    foreach (ProcessingNode curPN in FindAllProcesses_withOutput(s.thing.id))
                    {
                        ...
                        {
                            if (LinkProductToInput(pn, s.thing))
                            {
                                simplified = true;
                            }
                        }
