...
namespace MadTomDev.UI
{
    public class QuickGraphics
    {
        ...
        public class Image
        {
            ...
            public static System.Windows.Media.Color GetColor(BitmapSource img, int x, int y)
            {
                if (x < img.Width && y < img.Height)
                {
                    var croppedBitmap = new CroppedBitmap(
                        img, new System.Windows.Int32Rect((int)x, (int)y, 1, 1));
            ...
            {
                Bitmap? bitmap = FromClipboard_Bitmap();
                if (bitmap != null)
            ...
            public static Bitmap? FromDragDrop_Bitmap(System.Windows.IDataObject dropedData)
            {
                Bitmap? result = null;
                try
                {
                    var formats = dropedData.GetFormats(true);
                    ...
                    {
                        string[] files = (string[])(dropedData.GetData(DataFormats.FileDrop));
                        if (files.Length == 1)
                        {
                            using (FileStream fs = new FileStream(files[0], FileMode.Open, FileAccess.Read))
                            {
                                result = (Bitmap)Bitmap.FromStream(fs);
                            }
                        }
                    }
                    else if (formats.Contains("PNG"))
                    {
                        using (MemoryStream ms = (MemoryStream)dropedData.GetData("PNG"))
                        {
                            ms.Position = 0;
                    ...
                    else if (first == DataFormats.Bitmap || formats.Contains("text/_moz_htmlinfo"))
                    {
                        ...
                        result = BitmapSourceToBitmap(src);
                    }
                    else if (formats.Contains("System.Windows.Media.Imaging.BitmapSource"))
                    {
                        var src = System.Windows.Clipboard.GetImage();
                        result = BitmapSourceToBitmap(src);
                    }
                    ...
                    {
                        result = (Bitmap)dropedData.GetData("System.Drawing.Bitmap");
                    }
                    // not working, drag from web-page returns null(error)
                    else if (formats.Contains("DragImageBits"))
                    {
                        using (MemoryStream imageStream = dropedData.GetData("DragImageBits") as MemoryStream)
                        {
                            ...
                            if (imageStream != null)
                            {
                                ...
                                using (BinaryReader br = new BinaryReader(imageStream))
                                {
                                    ...
                                    // error here
                                    result = Bitmap.FromHbitmap(shDragImage.hbmpDragImage);
                                }
                            }
                        }
                    }
            ...
            public static BitmapSource? FromDragDrop(System.Windows.IDataObject dropedData)
            {
                Bitmap? bitmap = FromDragDrop_Bitmap(dropedData);
                if (bitmap != null)
                    return BitmapToBitmapSource(bitmap);
                else
                    return null;
            ...
            public static Bitmap GetGrayImage(Bitmap source, bool changeSource = false)
            {
                Bitmap result;
                if (changeSource)
                {
                ...
                int bytes = result.Width * result.Height * 3;
            ...
            public static BitmapSource GetGrayImage(BitmapSource source, bool changeSource = false)
            {
                Bitmap img = GetGrayImage(BitmapSourceToBitmap(source), true);
                if (changeSource)
                {
                    source = BitmapToBitmapSource(img);
                    return source;
                }
                else
                {
            ...
            public static Bitmap GetTransparentImage(Bitmap source, float opacity, bool changeSource = false)
            {
                Bitmap result = new Bitmap(source.Width, source.Height); // Determining Width and Height of Source Image
            ...
            public static BitmapSource GetTransparentImage(BitmapSource source, float opacity, bool changeSource = false)
            {
                Bitmap img = GetTransparentImage(BitmapSourceToBitmap(source), opacity, true);
                if (changeSource)
                {
                    source = BitmapToBitmapSource(img);
                    return source;
        ...
        public class Shape
        {
            ...
            public static void DrawArrowC1(ref WriteableBitmap bitmapSource,
                ...
                float lengthBottomRate = 3, float maxBottomWidth = 5)
            {
                Bitmap bitmap = BitmapSourceToBitmap(bitmapSource);
                using (Graphics g = Graphics.FromImage(bitmap))
                {
                    DrawArrowC1(
                        g,
                        WinPointToPointF(startPoint),
                        WinPointToPointF(endPoint),
                        WinColorToColor(fillColor),
                        WinColorToColor(borderColor)
                        );
                }
                bitmapSource = BitmapToBitmapSource(bitmap);
            }

            public static void DrawArrowC1(Graphics g, PointF startPoint, PointF endPoint,
                Color fillColor, Color borderColor, float lengthBottomRate = 3, float maxBottomWidth = 5)
            {
                double length = QuickCheckNCalculate.CalculateDistance(startPoint, endPoint);
                double bWidth = length / lengthBottomRate;
                if (bWidth > maxBottomWidth)
                    bWidth = maxBottomWidth;

                double halfBW = bWidth / 2;
                double halfAng
                    = Math.Atan2(endPoint.Y - startPoint.Y, endPoint.X - startPoint.X)
                        + Math.PI / 2;
                halfAng = -halfAng;
                double disX = Math.Cos(halfAng) * halfBW;
                double disY = Math.Sin(halfAng) * halfBW;
                PointF bp1 = new PointF((float)(startPoint.X + disX), (float)(startPoint.Y - disY));
                PointF bp2 = new PointF((float)(startPoint.X - disX), (float)(startPoint.Y + disY));
                PointF[] arrowPolygon = new PointF[] { bp1, bp2, endPoint };


                if (fillColor != Color.Empty && fillColor.A > 0)
                {
                    using (Brush bru = new SolidBrush(fillColor))
                    {
                        g.FillPolygon(bru, arrowPolygon);
                    }
                }
                if (borderColor != Color.Empty && borderColor.A > 0)
                {
                    using (Brush penBru = new SolidBrush(borderColor))
                    using (Pen pen = new Pen(penBru))
                    {
                        g.DrawPolygon(pen, arrowPolygon);
                    }
                }
            }
        ...
        public static BitmapSource ChopImage_RemoveBlankSpace(BitmapSource oriImage, int alphaThreshold = 0)
        {
            Bitmap bitmap = BitmapSourceToBitmap(oriImage);
            return BitmapToBitmapSource(ChopImage_RemoveBlankSpace_Bitmap(bitmap, alphaThreshold));
        }
        public static Bitmap ChopImage_RemoveBlankSpace_Bitmap(Bitmap oriImage, int alphaThreshold = 0)
        {
            Rectangle area = QuickCheckNCalculate.CheckImageContents(oriImage, alphaThreshold);

            return ChopImage_Bitmap(
                oriImage,
                area.Left,
        ...
        public class Screen
        {
            ...
            public static Bitmap GetCurrentScreenShot_Bitmap(Window window)
            {
                ...
                System.Windows.Forms.Screen curScreen = System.Windows.Forms.Screen.FromRectangle(windowArea);
                Rectangle scrBnds = curScreen.Bounds;
                ...
                using (var bmpGraphics = Graphics.FromImage(screenBmp))
                {
                    bmpGraphics.CopyFromScreen(scrBnds.X, scrBnds.Y, 0, 0, screenBmp.Size);
                }
                return screenBmp;

            }
            public static BitmapSource GetCurrentScreenShot(Window window)
            {
                return QuickGraphics.BitmapToBitmapSource(GetCurrentScreenShot_Bitmap(window));
            }

            public static Bitmap GetAllInOneScreenShot_Bitmap(out Rectangle allInOneBounds)
            {
                int left = 0, top = 0, right = 0, bottom = 0;
                Rectangle bnds;
                foreach (System.Windows.Forms.Screen curScreen in System.Windows.Forms.Screen.AllScreens)
                {
                    bnds = curScreen.Bounds;
                    if (bnds.X < left) left = bnds.X;
                    if (bnds.Y < top) top = bnds.Y;
                    if (right < bnds.Right) right = bnds.Right;
                    if (bottom < bnds.Bottom) bottom = bnds.Bottom;
                }
                allInOneBounds = new Rectangle(left, top, right - left, bottom - top);
                Bitmap screenBmp = new Bitmap(
                    allInOneBounds.Width,
                    allInOneBounds.Height,
                    System.Drawing.Imaging.PixelFormat.Format32bppArgb);
            ...
            public static BitmapSource GetAllInOneScreenShot(out Rectangle allInOneBounds)
            {
                return QuickGraphics.BitmapToBitmapSource(GetAllInOneScreenShot_Bitmap(out allInOneBounds));
            }
        }
        ...
        #region
        //public static BitmapSource GenerateSizedText(string text,
        //    string fontfamily,
        //    double fontSize,
        //    System.Windows.Media.Brush brush, float maxWidth, float maxHeight,
        //    bool isBold = false, bool isItalic = false, bool isUnderline = false, bool isStrikeout = false)
        //{
        ...
        //    Font font = new Font(new FontFamily(fontfamily), (float)fontSize, fontStyle);
        //}
        public static Bitmap GenerateSizedText(string text, Font font, Brush brush, float maxWidth, float maxHeight)
        {
            SizeF txSize;
            Graphics tMeasure = Graphics.FromHwnd(IntPtr.Zero);
            txSize = tMeasure.MeasureString(text, font);

            Bitmap testImg = new Bitmap((int)txSize.Width + 1, (int)txSize.Height + 1);
            using (Graphics g = Graphics.FromImage(testImg))
            using (Brush b = new SolidBrush(Color.Black))
            {
                g.DrawString(text, font, b, 0, 0);
            }
            Bitmap testImg2 = ChopImage_RemoveBlankSpace_Bitmap(testImg);
            testImg.Dispose();

            float ratioW = maxWidth / testImg2.Width;
            float ratioH = maxHeight / testImg2.Height;
            testImg2.Dispose();
            ...
            using (Graphics g2 = Graphics.FromImage(testImg))
                g2.DrawString(text, font, brush, 0, 0);

            testImg2 = ChopImage_RemoveBlankSpace_Bitmap(testImg);
            testImg.Dispose();
            return testImg2;
        }
        #endregion
